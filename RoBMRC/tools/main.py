...
#     for i in range(len(sentence_with_pos)):
#         if (sentence_with_pos[i][1] in target_list):
#             target_list.append(sentence_with_pos[i][0])
#             target_list_index.append(i)
#     #print("target_list", target_list)
#     #print("index", target_list_index)

#     start_index = []
#     end_index = []

#     for i in range (0, len(target_list_index)-1):
#         if target_list_index[i] == (target_list_index[i+1]-1):
#             start_index.append(target_list_index[i])
#         if target_list_index[i] == opinion_list_index[i-1] +1:
#             end_index.append(target_list_index[i])
#     for i in range(len(target_list_index)):
#         if target_list_index[i] not in start_index:
#             if target_list_index[i] not in end_index:
#                 start_index.append(target_list_index[i])
#                 end_index.append(target_list_index[i])
...
def test(model, tokenizer, batch_generator, test_data, beta, logger, gpu, max_len):
    ...
    for batch_index, batch_dict in enumerate(batch_generator):
        ...
        #print(batch_dict['line'][0])

        ok_start_index = batch_dict['forward_asp_answer_start'][0].gt(
            -1).float().nonzero()  # indices qui sont superieurs à -1 (juste la sequence, en eliminant la query)
        #print("ok_start_index", ok_start_index)
        ...
        for i in range(len(f_asp_end_scores)):
            for j in range(len(f_asp_end_scores[0])):
                f_asp_end_scores[i][j][1] += (batch_dict['forward_asp_prob'][i][j] +
                                                   batch_dict['forward_aspect_prob'][i][j])
                f_asp_end_scores[i][j][0] += batch_dict['forward_asp_neg_prob'][i][j]
        ...
        f_asp_start_prob, f_asp_start_ind = torch.max(
            f_asp_start_scores, dim=1)  # "torch.max" pour extraire la probabilité et l'indice de début les plus élevés pour chaque aspect dans les requêtes d'aspects.
        f_asp_end_prob, f_asp_end_ind = torch.max(f_asp_end_scores, dim=1)

        #print("f_asp_start_ind,", f_asp_start_ind)
        ...
        for start_index in range(f_asp_start_ind.size(0)):
            if batch_dict['forward_asp_answer_start'][0, start_index] != -1:
                if f_asp_start_ind[start_index].item() == 1:
                    ...
                    f_asp_start_prob_temp.append(
                        f_asp_start_prob[start_index].item())
                if f_asp_end_ind[start_index].item() == 1:
                    f_asp_end_index_temp.append(start_index)
                    f_asp_end_prob_temp.append(
                        f_asp_end_prob[start_index].item())

       # print("f_asp_start_prob_temp, f_asp_end_prob_temp,f_asp_start_index_temp, f_asp_end_index_temp ",
        ...
        #print("f_asp_start_index_final, f_asp_end_index_final, f_asp_prob_final",
        #  f_asp_start_index, f_asp_end_index, f_asp_prob)
        ...
        for start_index in range(len(f_asp_start_index)):
            ...
            for j in range(f_asp_start_index[start_index], f_asp_end_index[start_index] + 1):
                opinion_query.append(
                    batch_dict['forward_asp_query'][0][j].item())
                aspects_list.append(tokenizer.convert_ids_to_tokens(
                    batch_dict['forward_asp_query'][0][j].item()))
            ...
            opinion_query = torch.cat(
                [opinion_query, ok_start_tokens], -1).unsqueeze(0)
            #print("opinion_query_afetr_cat", opinion_query)
            opinion_query_seg += [1] * ok_start_tokens.size(0)
            opinion_query_mask = torch.ones(
                opinion_query.size(1)).float().unsqueeze(0)
            #print("opinion_query_mask",opinion_query_mask)
            if gpu:
                opinion_query_mask = opinion_query_mask.cuda()
            opinion_query_seg = torch.tensor(
                opinion_query_seg).long().unsqueeze(0)
            #print("opinion_query_seg",opinion_query_seg)
            if gpu:
            ...
            for k in range(f_opi_start_ind.size(0)):
                if opinion_query_seg[0, k] == 1:
                    if f_opi_start_ind[k].item() == 1:
                        ...
                        f_opi_start_prob_temp.append(
                            f_opi_start_prob[k].item())
            ...
            for idx in range(len(f_opi_start_index)):
                ...
                opi = [opinion_query[0][j].item() for j in range(
                    f_opi_start_index[idx], f_opi_end_index[idx] + 1)]
                asp_ind = [f_asp_start_index[start_index] -
                           5, f_asp_end_index[start_index] - 5]
                opi_ind = [f_opi_start_index[idx] - f_opi_length,
                           f_opi_end_index[idx] - f_opi_length]
                # TODO
                temp_prob = math.sqrt(
                    f_asp_prob[start_index] * f_opi_prob[idx])
        ...
        for i in range(len(b_opi_end_scores)):
                    for j in range(len(b_opi_end_scores[0])):
                        b_opi_end_scores[i][j][1] += (batch_dict['backward_opi_prob'][i][j] + batch_dict['backward_opinion_prob'][i][j])
                        b_opi_end_scores[i][j][0] += batch_dict['backward_opi_neg_prob'][i][j]

        b_opi_start_scores = F.softmax(b_opi_start_scores[0], dim=1)
        b_opi_end_scores = F.softmax(b_opi_end_scores[0], dim=1)
        b_opi_start_prob, b_opi_start_ind = torch.max(
            b_opi_start_scores, dim=1)
        ...
        for start_index in range(b_opi_start_ind.size(0)):
            if batch_dict['backward_opi_answer_start'][0, start_index] != -1:
                if b_opi_start_ind[start_index].item() == 1:
                    ...
                    b_opi_start_prob_temp.append(
                        b_opi_start_prob[start_index].item())
                if b_opi_end_ind[start_index].item() == 1:
                    b_opi_end_index_temp.append(start_index)
                    b_opi_end_prob_temp.append(
                        b_opi_end_prob[start_index].item())
        ...
        for start_index in range(len(b_opi_start_index)):
            ...
            aspect_query = torch.cat(
                [aspect_query, ok_start_tokens], -1).unsqueeze(0)
            aspect_query_seg += [1] * ok_start_tokens.size(0)
            aspect_query_mask = torch.ones(
                aspect_query.size(1)).float().unsqueeze(0)
            if gpu:
                aspect_query_mask = aspect_query_mask.cuda()
            aspect_query_seg = torch.tensor(
                aspect_query_seg).long().unsqueeze(0)
            if gpu:
            ...
            b_asp_start_prob, b_asp_start_ind = torch.max(
                b_asp_start_scores, dim=1)
            ...
            for k in range(b_asp_start_ind.size(0)):
                if aspect_query_seg[0, k] == 1:
                    if b_asp_start_ind[k].item() == 1:
                        ...
                        b_asp_start_prob_temp.append(
                            b_asp_start_prob[k].item())
            ...
            for idx in range(len(b_asp_start_index)):
                ...
                asp = [aspect_query[0][j].item() for j in range(
                    b_asp_start_index[idx], b_asp_end_index[idx] + 1)]
                asp_ind = [b_asp_start_index[idx] - b_asp_length,
                           b_asp_end_index[idx] - b_asp_length]
                opi_ind = [b_opi_start_index[start_index] -
                           5, b_opi_end_index[start_index] - 5]
                # TODO
                temp_prob = math.sqrt(
                    b_asp_prob[idx] * b_opi_prob[start_index])
            ...
            if forward_pair_list[idx] in backward_pair_list or forward_pair_prob[idx] >= beta:
                if forward_pair_list[idx][0] not in final_asp_list:
                    ...
                    if forward_pair_list[idx][1] not in final_opi_list[asp_index]:
                        final_opi_list[asp_index].append(
                            forward_pair_list[idx][1])
                        final_opi_ind_list[asp_index].append(
                            forward_pair_ind_list[idx][2:])
        # backward
            ...
            if backward_pair_list[idx] not in forward_pair_list:
                if backward_pair_prob[idx] >= beta:
                    if backward_pair_list[idx][0] not in final_asp_list:
                        ...
                        final_asp_ind_list.append(
                            backward_pair_ind_list[idx][:2])
                        final_opi_ind_list.append(
                            [backward_pair_ind_list[idx][2:]])
                    else:
                        asp_index = final_asp_list.index(
                            backward_pair_list[idx][0])
                        if backward_pair_list[idx][1] not in final_opi_list[asp_index]:
                            final_opi_list[asp_index].append(
                                backward_pair_list[idx][1])
                            final_opi_ind_list[asp_index].append(
                                backward_pair_ind_list[idx][2:])
        ...
        for idx in range(len(final_asp_list)):
            ...
            for idy in range(predict_opinion_num):
                ...
                sentiment_query = torch.cat(
                    [sentiment_query, ok_start_tokens], -1).unsqueeze(0)
                sentiment_query_seg += [1] * ok_start_tokens.size(0)
                sentiment_query_mask = torch.ones(
                    sentiment_query.size(1)).float().unsqueeze(0)
                if gpu:
                    sentiment_query_mask = sentiment_query_mask.cuda()
                sentiment_query_seg = torch.tensor(
                    sentiment_query_seg).long().unsqueeze(0)
                if gpu:
                    sentiment_query_seg = sentiment_query_seg.cuda()

                sentiment_scores = model(
                    sentiment_query, sentiment_query_mask, sentiment_query_seg, 'S')
                sentiment_predicted = torch.argmax(
                    sentiment_scores[0], dim=0).item()
    ...
    logger.info(
        'Triplet - Precision: {}\tRecall: {}\tF1: {}'.format(precision, recall, f1))
    ...
    f1_aspect = 2 * precision_aspect * recall_aspect / \
        (precision_aspect + recall_aspect + 1e-6)
    logger.info('Aspect - Precision: {}\tRecall: {}\tF1: {}'.format(
        precision_aspect, recall_aspect, f1_aspect))
    ...
    f1_opinion = 2 * precision_opinion * recall_opinion / \
        (precision_opinion + recall_opinion + 1e-6)
    logger.info('Opinion - Precision: {}\tRecall: {}\tF1: {}'.format(
        precision_opinion, recall_opinion, f1_opinion))

    precision_aspect_sentiment = float(
        asp_sent_match_num) / float(asp_sent_predict_num + 1e-6)
    recall_aspect_sentiment = float(
        asp_sent_match_num) / float(asp_sent_target_num + 1e-6)
    ...
    precision_aspect_opinion = float(
        asp_opi_match_num) / float(asp_opi_predict_num + 1e-6)
    recall_aspect_opinion = float(
        asp_opi_match_num) / float(asp_opi_target_num + 1e-6)
...
def create_directory(arguments):
    ...
    log_path = arguments.log_path + dataset_version + \
        arguments.data_name + arguments.model_name + '.log'
    model_path = arguments.save_model_path + dataset_version + \
        arguments.data_name + arguments.model_name + '.pth'
...
def train(arguments):
    ...
    tokenizer = BertTokenizer.from_pretrained(arguments.bert_model_type)
    ...
    if arguments.mode == 'test':
        ...
        batch_generator_test = Data.generate_batches(dataset=test_dataset, batch_size=1, shuffle=False,
                                                     gpu=arguments.gpu)
        test(model, tokenizer, batch_generator_test, test_standard, arguments.inference_beta, logger,
        ...
        optimizer = AdamW(optimizer_grouped_parameters,
                          lr=arguments.tuning_bert_rate, correct_bias=False)
        ...
        if arguments.reload and os.path.exists(model_path):
            ...
            logger.info('Reload model and optimizer after training epoch {}'.format(
                checkpoint['epoch']))
        else:
            start_epoch = 1
        ...
        for epoch in range(start_epoch, arguments.epoch_num + 1):
            ...
            for batch_index, batch_dict in enumerate(batch_generator):
                ...
                f_aspect_start_scores, f_aspect_end_scores = model(batch_dict['forward_asp_query'],
                                                                   batch_dict['forward_asp_query_mask'],
                                                                   batch_dict['forward_asp_query_seg'], 'A')
                for i in range(len(f_aspect_end_scores)):
                    for j in range(len(f_aspect_end_scores[0])):
                        f_aspect_end_scores[i][j][1] += ( batch_dict['forward_asp_prob'][i][j] + \
                            batch_dict['forward_aspect_prob'][i][j])
                        f_aspect_end_scores[i][j][0] += batch_dict['forward_asp_neg_prob'][i][j]       
                ...
                for i in range(len(b_opi_end_scores)):
                    for j in range(len(b_opi_end_scores[0])):
                        b_opi_end_scores[i][j][1] += (
                            batch_dict['backward_opi_prob'][i][j] + batch_dict['backward_opinion_prob'][i][j])
                        b_opi_end_scores[i][j][0] += batch_dict['backward_opi_neg_prob'][i][j]
                ...
                batch_dict['forward_opinion_prob'] = batch_dict['forward_opinion_prob'] \
                    .view(-1, batch_dict['forward_opinion_prob'].size(-1))
                ...
                # for i in range(len(f_opi_start_scores)):
                #     for j in range(len(f_opi_start_scores[0])):
                #         f_opi_start_scores[i][j][1] += batch_dict['forward_opinion_prob'][i][j]
                ...
                batch_dict['backward_aspect_prob'] = batch_dict['backward_aspect_prob'] \
                    .view(-1, batch_dict['backward_aspect_prob'].size(-1))
                ...
                # for i in range(len(b_asp_start_scores)):
                #     for j in range(len(b_asp_start_scores[0])):
                #         b_asp_start_scores[i][j][1] += batch_dict['backward_aspect_prob'][i][j]
                ...
                if (batch_index + 1) % 10 == 0:
                    logger.info('Epoch:[{}/{}]\t Batch:[{}/{}]\t Loss Sum:{}\t '
                                ...
                                format(epoch, arguments.epoch_num, batch_index + 1, batch_num_train,
                                       ...
                                       round(f_asp_loss.item(), 4), round(
                                           f_opi_loss.item(), 4),
                                       round(b_opi_loss.item(), 4), round(
                                           b_asp_loss.item(), 4),
            ...
            batch_generator_dev = Data.generate_batches(dataset=dev_dataset, batch_size=1, shuffle=False,
                                                        gpu=arguments.gpu)
            logger.info("dev")
            dev_f1 = test(model, tokenizer, batch_generator_dev, dev_standard, arguments.inference_beta, logger,
                          arguments.gpu, max_len)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Bidirectional MRC-based sentiment triplet extraction')
    parser.add_argument('--data_path', type=str,
                        default="./data/preprocess/" + dataset_version)
    parser.add_argument('--data_name', type=str,
                        default=dataset_name_list[0], choices=dataset_name_list)
    ...
    parser.add_argument('--train', type=str,
                        default=dataset_type_list[0], choices=dataset_type_list)
    parser.add_argument(
        '--dev', type=str, default=dataset_type_list[1], choices=dataset_type_list)
    parser.add_argument(
        '--test', type=str, default=dataset_type_list[2], choices=dataset_type_list)
    parser.add_argument('--max_len', type=str,
                        default="max_len", choices=["max_len"])
    parser.add_argument('--max_aspect_num', type=str,
                        default="max_aspect_num", choices=["max_aspect_num"])

    parser.add_argument('--reload', type=bool, default=False)

    parser.add_argument('--bert_model_type', type=str,
