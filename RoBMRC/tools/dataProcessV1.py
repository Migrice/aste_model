...
def make_QA(line, word_list, aspect_list, opinion_list, sentiment_list):
    ...
    for i in range(len(aspect_list)):
        ...
        sentiment_query_seg_temp = [
            0] * (len(sentiment_query_temp) - len(word_list)) + [1] * len(word_list)
...
def train_data_process(text, triplet_data):
    ...
    for i in range(len(text)):
        line = text[i]
        QA, max_aspect_temp, max_len_temp = line_data_process(
            line, triplet_data[i])
...
def dataset_process():
    for dataset_name in dataset_name_list:
        ...
        max_len = 0
        for dataset_type in dataset_type_list:
            text_file = open(data_path + dataset_name + "/" +
            ...
            QA_list, max_aspect_temp, max_len_temp = train_data_process(
                text_lines, triplet_data)
            train_dataset_object[dataset_type], max_tokens_temp = tokens_to_ids(
                QA_list)
            # TODO
            test_dataset_object[dataset_type] = test_data_process(
                text_lines, triplet_data)
